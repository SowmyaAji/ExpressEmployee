...
The app uses Node.js and its framework Express.js to create, retrieve, update and destroy records of employees. It also uses two external APIs to generate quotes and jokes for every employee, which stay the same when other details of the employee are updated. An id is also autogenerated incrementally for each new employee, but stays the same when updated. 
...
This threw up the error coded in the validation function: 
